...
@section('content')
    <div class="container mx-auto py-6">
        <div class="bg-white p-6 rounded-lg shadow-lg">
            <h2 class="text-2xl font-semibold text-gray-800 mb-4">Edit Produk</h2>
            ...
            <form action="{{ route('admin.product.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" id="name" class="mt-2 p-3 border border-gray-300 rounded-lg w-full" value="{{ old('name', $product->name) }}" required>
                    @error('name')
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" rows="4" class="mt-2 p-3 border border-gray-300 rounded-lg w-full">{{ old('description', $product->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="price" id="price" class="mt-2 p-3 border border-gray-300 rounded-lg w-full" value="{{ old('price', $product->price) }}" required>
                    @error('price')
                        <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="stock" id="stock" class="mt-2 p-3 border border-gray-300 rounded-lg w-full" value="{{ old('stock', $product->stock) }}" required>
                    @error('stock')
                        <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    @enderror
                    @if($product->image)
                        <img src="{{ Storage::url($product->image) }}" alt="Product Image" class="mt-4 w-32 h-32 object-cover">
                    @endif
                </div>
